	...
	#print @sd
		...
		jc=false
			...
			if @wy==true
				if c[q]==')'
					rpn =0
					if @wyl.length>1 && (rpn = [ @wyl[2..-1].max , -@wyl[2..-1].min ].max ) >= 1024
						rpn = ( rpn+3 ) / 4 
						mm<<[[205],0,rpn,0]
					end
					case @wyl.length
					...
					when 1
						16.times { |n| 
						mm<<[[204],@wyl[0].to_f/2048,(8192*n)*256/rpn+8192,0] }
					when 2
						16.times { |n|
						mm<<[[204],@wyl[0].to_f/2048,8192+(@wyl[1]*n/2)*256/rpn,0]  }
						mm<<[[204],@wyl[0].to_f/2048,8192,0]
					else
						(@wyl.length-2).times{ |nn|
							16.times{ |n|
								mm<<[[204],@wyl[0].to_f/(2048*(@wyl.length-2)),8192+((@wyl[1+nn]*(16-n)+@wyl[2+nn]*n)/2)*256/rpn,0]
							}
						}
