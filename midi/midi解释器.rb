...
class TextTranslator < MIDI::IO::MIDIFile
  ...
  def note_on(chan, note, vel)
    ...
    $noon[-1]<<[@curr_ticks,chan,note,vel]
  ...
  def pitch_bend(chan, msb, lsb)
    ...
    $pcbb=$pcb
    $pcb=lsb
    if ($pcbb>$pcb && $pcb<lsb || $pcbb<$pcb && $pcb>lsb ) && pcbb>-1
      @pending.each_with_index do |num, i|                                  #弯音轮突出事件
      end
    end
    track_uses_channel(chan)
  end
  ...
  def tempo(microsecs)
    pdelta()
    bpm = 60000000.0 / microsecs # quarter notes per microsecond
